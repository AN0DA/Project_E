...
void env_temperature::generate_temperature(sprite_params** data, int width, int height) {
	...
	for (int i = 1; i < width; i++) {
		std::uniform_int_distribution<> dis((data[0][i - 1].get_temperature() * change_percent), (data[0][i - 1].get_temperature() * (change_percent + 1)));
		data[0][i].set_temperature(dis(gen));
	}

	//generate first column
	for (int i = 1; i < height; i++) {
		std::uniform_int_distribution<> dis((data[i - 1][0].get_temperature() * change_percent), (data[i - 1][0].get_temperature() * (change_percent + 1)));
